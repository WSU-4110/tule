...
function algorithm(inputTasks) {
    ...
    // scheduling the tasks with a start time, then scheduling tasks one by one,
    // first by priority, then by duration. The tasks with the longest duration
    // get scheduled first.
    function dailyAlgorithm(taskList, schedStart = "08:00", schedEnd = "24:00",
            first = false) {
        ...
        if (first) {
            for (let i = 0; i < taskList.length; i++) {
                ...
                dailySchedule.push([taskStart, taskList[i].Name, taskEndTime]);
            }
        ...
        // it will come here and finds a place to schedule each task.
            ...
            for (let i = 0; i < taskList.length; i++) {
                ...
                while (!success && (x < dailySchedule.length)) {
                    // This checks to see if there is a next task in the schedule.
